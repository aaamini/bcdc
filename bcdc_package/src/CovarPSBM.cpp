...
CovarPSBM::CovarPSBM(const arma::sp_mat& A_, const arma::sp_mat& mask_,
            const double alpha_eta, const double beta_eta,
            const double dp_concent_param)
            : BasicPSBM::BasicPSBM(A_, mask_), dp_concent_param{dp_concent_param}
{
...
void CovarPSBM::set_continuous_features(const arma::mat& X_)
{
    Xc = X_;
...
void CovarPSBM::set_discrete_features(const arma::umat& X_)
{
    Xd = X_;
...
void CovarPSBM::set_gauss_param(const double s2_, const double tau2_)
{
    s2 = s2_;
    tau2 = tau2_;
}

void CovarPSBM::set_beta_params(const double alpha_eta, const double beta_eta)
{
    // beta_params.alpha = alpha_eta;
    // beta_params.beta = beta_eta;
...
// void CovarPSBM::update_dist_mat() {
//     // for (int i = 0; i < n; i++) {
...
void CovarPSBM::repopulate_from_prior(){
...
void CovarPSBM::update_z_element(const int i) {


    arma::vec taui = col_adj_compress(i); // sp_single_col_compress(A, i, z, K);
    arma::vec mmi = col_mask_compress(i);
...
arma::umat CovarPSBM::run_gibbs(const int niter) {

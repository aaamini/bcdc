...
BasicPSBM::BasicPSBM(const arma::sp_mat& A_, const arma::sp_mat& mask_)
{
    mask = mask_;
    A = A_ % mask;
    n = A.n_rows;
...
BasicPSBM::BasicPSBM(const arma::sp_mat& A_, const arma::sp_mat& mask_, const int K) : K{K}
{
    ...
    N = arma::mat(K, K, arma::fill::zeros);
    set_z_to_random_labels();

    // Rcpp::print(wrap( blk_compressions[1]));
}

void BasicPSBM::set_z_to_random_labels()
{
    z = sample_int_vec(K, n);
}

arma::vec BasicPSBM::col_adj_compress(const int& col_idx)
{
...
arma::vec BasicPSBM::col_mask_compress(const int& col_idx)
{
    arma::vec b(K, arma::fill::zeros);
    for (arma::sp_mat::const_col_iterator it = mask.begin_col(col_idx); it != mask.end_col(col_idx); ++it) {
        b(z(it.row())) += (*it);
    }
    return b;
}

arma::uvec BasicPSBM::get_label_freq()
{
...
arma::uvec BasicPSBM::get_label_freq_except(const int& i) {
...
void BasicPSBM::update_blk_sums_and_sizes() {
    ...
    arma::sp_mat::const_iterator mit     = mask.begin();
    arma::sp_mat::const_iterator mit_end = mask.end();

    N = arma::mat(K, K, arma::fill::zeros);
    for(; mit != mit_end; ++mit) {
        N(z(mit.row()), z(mit.col())) += (*mit);
    }
...
// void BasicPSBM::update_z_element(const int i) {
...
// arma::umat BasicPSBM::run_gibbs(const int niter) {
...
//         BasicPSBM(const arma::sp_mat& A_,
...
//     BasicPSBM mynsbm(A, K, L);

//     mynsbm.run_gibbs(100);
// }

// RCPP_MODULE(basic_sbm_module) {
//       class_<BasicPSBM>("BasicPSBM")
...
//       .method("col_compress", &BasicPSBM::col_compress)
// //      .method("print", &BasicPSBM::print)
//     //   .method("update_z_element", &BasicPSBM::update_z_element)
//     //   .method("run_gibbs", &BasicPSBM::run_gibbs)
//     //   .method("update_pri", &BasicPSBM::update_pri)
//       ;
// };

// RCPP_EXPOSED_CLASS(BasicPSBM);

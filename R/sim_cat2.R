...
simdata <- function(n, p, r) {
  ...
  feature <- cbind(c(sample(seq_len(4), n1, prob = dir_p1, replace = TRUE)
                     , sample(seq_len(4), n2, prob = dir_p2, replace = TRUE))
                   , sample(seq_len(4), n, prob = rep(1/4, 4), replace = TRUE))
...
for (tt in seq_len(nrow(grid))) {
  ...
  res <- simplify2array(
    pbmclapply(seq_len(n_reps), mc.cores = n_cores, function(i)
    {
      ...
      Z_true <- cbind(seq_len(n)
                      , c(rep(1, simnum[1]), rep(2, simnum[2])))
